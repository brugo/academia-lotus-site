    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-serif text-slate-100 flex items-center gap-3">
            ...
            Construtor de Páginas ({pageRoute === 'home' ? 'Home' : 'Cursos'})
          </h1>
          <p className="text-slate-400 mt-2 font-light">Gerencie os blocos que compõem a página {pageRoute === 'home' ? 'Home' : 'Cursos'}. Arraste para reordenar.</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-midnight-900/30 border border-white/5 border-dashed rounded-3xl p-12 text-center text-slate-400">
          ...
          <p className="mb-6 font-light">Comece adicionando seu primeiro bloco arquitetural para a página {pageRoute}.</p>
          <button 
            onClick={() => {
              setEditingBlock(null);
              setSelectedTypeForNew('hero');
